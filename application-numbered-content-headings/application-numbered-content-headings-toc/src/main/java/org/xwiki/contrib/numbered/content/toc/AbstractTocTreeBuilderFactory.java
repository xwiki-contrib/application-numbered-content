...
public abstract class AbstractTocTreeBuilderFactory implements TocTreeBuilderFactory
{
    ...
    protected TocEntriesResolver getTocEntriesResolver(String resolverHint) throws ComponentLookupException
    {
        ...
        // The component must succeed to be resolved only if a hint is explicitly requested (i.e., resolverHint is not 
        // null).
        if (resolverHint != null) {
            tocEntriesResolver = this.componentManager.getInstance(TocEntriesResolver.class, resolverHint);
        } else if (this.componentManager.hasComponent(TocEntriesResolver.class)) {
            tocEntriesResolver = this.componentManager.getInstance(TocEntriesResolver.class);
        } else {
            tocEntriesResolver = null;
        }
        return tocEntriesResolver;
    }
