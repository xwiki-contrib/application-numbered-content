...
/**
 * Used to generate the table of contents for the PDF export. Note that this class is a copy of
 * {@code org.xwiki.export.pdf.internal.macro.PDFTocMacro}.
...
public class PDFTocMacro extends AbstractMacro<PDFTocMacroParameters>
{
    ...
    {
        List<String> jobId = List.of(jobIdString.split("/"));
    ...
    private XDOM aggregateRenderingResults(List<PDFExportJobStatus.DocumentRenderingResult> renderingResults)
    {
        return new XDOM(
            renderingResults.stream().map(PDFExportJobStatus.DocumentRenderingResult::getXDOM).map(XDOM::getChildren)
    ...
    private List<Block> generateToc(XDOM content, int depth, MacroTransformationContext context)
    {
        ...
        TreeParameters treeParameters = new TreeParametersBuilder().build(content, parameters, context);
